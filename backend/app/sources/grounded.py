...
A wrapper rather than a change to the engine: the demo assessor and the engine both get it, the
engine's own output (and its cache, and its tests) stay exactly what they were, and retrieval is
...
class GroundedAssessor:
    ...
    async def assess(self, route: Route, scenario: Scenario, day: date | None = None) -> AssessmentData:
        data = await self.inner.assess(route, scenario, day)
        hazards = []
