  ...
  useEffect(() => {
    ...
    const id = navigator.geolocation.watchPosition(
      ({ coords }) => {
        ...
        const snap = snapToTrack(track, [coords.latitude, coords.longitude], progressRef.current)
        if (snap === null) return
        if (snap.offRouteM > OFF_ROUTE_M) {
          setPosition((current) => ({ status: 'offRoute', snap: current.snap }))
          return
        }
        progressRef.current = Math.max(progressRef.current, snap.progressM)
        setPosition({ status: 'tracking', snap })
      },
      (error) => {
        setPosition((current) => ({
